...
class SingleFoldCrossValidator:
    ...
    def create_single_fold_split(self, stratify_by_change_ratio: bool = True):
        ...
        # Calculate change ratios for stratification if enabled
        if stratify_by_change_ratio:
            change_ratios = self._calculate_change_ratios(all_files)
            # Create stratification bins (low, medium, high change)
    ...
    def _create_stratification_bins(self, change_ratios: List[float]) -> List[int]:
        ...
        # Define bins: low (0-33%), medium (33-67%), high (67-100%)
        ...
        for ratio in ratios:
            if ratio <= percentiles[0]:
                bins.append(0)  # Low change
            elif ratio <= percentiles[1]:
                bins.append(1)  # Medium change
            else:
                bins.append(2)  # High change
...
class ImprovedTrainingVisualizer:
    ...
    def __init__(self, vis_dir, exp_name):
        ...
        self.cv_results_dir = self.vis_dir / "cv_results"  # New directory for CV results
        self.plots_dir.mkdir(exist_ok=True)
        self.predictions_dir.mkdir(exist_ok=True)
        self.postprocessed_dir.mkdir(exist_ok=True)
        self.augmented_samples_dir.mkdir(exist_ok=True)
        self.cv_results_dir.mkdir(exist_ok=True)  # Create CV results directory
...
class EnhancedCDTrainerWithCV(CDTrainer):
    ...
    def train_models(self):
        ...
        # Final validation to get final metrics
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.dataloaders['val']):
                try:
                    ...
                    # Progress logging for final evaluation or every few batches
...
def validate_args(args):
    ...
    if not 0.1 <= args.cv_validation_split <= 0.4:
        print(f"Validation split {args.cv_validation_split} is outside recommended range [0.1, 0.4]. Using 0.2")
...
def enhanced_train_with_cv(args):
    ...
    print("ENHANCED CHANGEFORMER TRAINING WITH SINGLE-FOLD CROSS-VALIDATION")
    print("=" * 80)
    ...
    # Initialize single-fold cross-validator and handle existing splits
    ...
    try:
        ...
        if val_list_path.exists() and train_list_path.exists():
            print("Using existing train.txt and val.txt splits")
            with open(train_list_path, 'r') as f:
            ...
            print(f"Single-fold CV initialized with {actual_split*100:.1f}% validation split")
        else:
        ...
        print(f"Using Enhanced Combined Loss with Cross-Validation:")
        ...
        print(f"   Cross-validation split: {args.cv_validation_split}")
        print("=" * 80)
        ...
        try:
            ...
            if 'val' in dataloaders:
                ...
                print("WARNING: No validation dataloader found!")
        except Exception as e:
        ...
        print("Starting single-fold cross-validation training...")
        ...
        print(f"Single-fold cross-validation training failed: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ChangeFormerV6_Enhanced_SingleFold_CV')
    ...
    parser.add_argument('--cv_stratify', type=bool, default=True,
                       help='Whether to stratify split by change pixel ratio')
    parser.add_argument('--restore_original_split', type=bool, default=False,
                       help='Whether to restore original split after training')
    
    # Enhanced checkpointing and visualization
    parser.add_argument('--checkpoint_root', type=str, default='./checkpoints/')
    parser.add_argument('--checkpoint_dir', type=str, 
                       default='./checkpoints/ChangeFormer_myCustom/ChangeFormerV6_Enhanced_SingleFold_CV')
    ...
    parser.add_argument('--augment_factor', type=int, default=1)  # No augmentation by default
    ...
    print("=" * 80)
    print("KEY IMPROVEMENTS FOR BETTER PREDICTION PERFORMANCE:")
    print(f"✓ Single-fold cross-validation with {args.cv_validation_split*100:.0f}% validation split")
    print(f"✓ Stratified sampling by change pixel ratio: {args.cv_stratify}")
    print(f"✓ Enhanced Loss: Focal Tversky ({args.focal_tversky_weight}) + Weighted BCE ({args.weighted_bce_weight})")
    print(f"✓ Tversky params: α={args.tversky_alpha} (FP penalty), β={args.tversky_beta} (FN penalty)")
    print(f"✓ Advanced data augmentation pipeline")
    print(f"✓ Comprehensive metrics tracking and visualization")
    print(f"✓ Best model selection based on validation performance")
    print("=" * 80)
    ...
    print(f"Random seed: {args.cv_random_seed}")
    print("=" * 80)
    ...
    try:
        ...
        print("\n" + "🎉" * 30)
        print("SINGLE-FOLD CROSS-VALIDATION TRAINING COMPLETED SUCCESSFULLY!")
        print("🎉" * 30)
        print("\nKEY PERFORMANCE METRICS:")
        print(f" Best Validation mF1: {fold_results['best_val_mf1']:.5f}")
        print(f"🎯 Best Change Detection F1: {fold_results['best_change_f1']:.5f}")
        print(f"📈 Best Change Detection IoU: {fold_results['best_change_iou']:.5f}")
        print(f"⏱️  Training Duration: {fold_results['training_duration']}")
        ...
        if fold_results['best_change_f1'] > 0.4:
            ...
            print("🔴 STATUS: NEEDS IMPROVEMENT - Consider tuning hyperparameters")
        
        print(f"\n📁 All results saved in: {args.vis_dir}/{args.exp_name}")
        print(f"🏆 Best model saved as: {args.checkpoint_dir}/best_cv_ckpt.pt")
        print("\n" + "=" * 80)
        
        # Provide recommendations for further improvement
        print("💡 RECOMMENDATIONS FOR FURTHER IMPROVEMENT:")
        if fold_results['best_change_f1'] < 0.3:
            print("- Consider increasing augmentation factor (--augment_factor 2)")
            print("- Try different loss weight combinations")
            print("- Experiment with different learning rates")
            print("- Increase training epochs if validation is still improving")
        elif fold_results['best_change_f1'] < 0.4:
            print("- Fine-tune Tversky loss parameters (alpha/beta)")
            print("- Consider ensemble methods for inference")
            print("- Implement test-time augmentation")
        else:
            print("- Excellent performance! Consider testing on held-out data")
